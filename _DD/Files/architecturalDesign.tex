...
\paragraph*{API call authorization check}
This interaction is intended as placed everywhere there is a communication between an external component, or client and the API manager.
...
Where the API call is any call as described above, and request is any action the Platform has to perform as result of that call.
...
For convenience this API is shown divided in three parts: fisrtly the part dedicated to the Web App, secondly the one dedicated to the E-Mail confirmations, thirdly the one dedicated to the RMP actions.
\subsubsection{Web App API}
\begin{itemize}
...
\end{itemize}
All of the following have a common argument, that is the User session token. This token is used to authenticate the User and to check if he has the right to perform the call. So all of the following calls are valid if and only if the User is logged in.
\begin{itemize}
    ...
    \item \textbf{GET /api/userProfile} This call allows a user to get all data that the Platform has about an user. The body of the request must contain the following fields: Email. It returns the user public data, and, in the case of Students, also the list of badges.
    ...
    \item \textbf{POST /api/inviteStudent} This call allows a Student to invite an other stuent to his team. The body of the request must contain the following fields: Tournament, TeamName, StudentEmail. It returns the confirmation of successful invitation, or an error message.
    \item \textbf{POST /api/inviteEducator} This call allows an Educator to invite an other Educator to his Tournament. The body of the request must contain the following fields: Tournament, EducatorEmail. It returns the confirmation of successful invitation, or an error message.
...
\textbf{User Session Token} This token is used to authenticate the User and to check if he has the right to perform the call. This is generated after a successful login and is valid for the whole session, or until the User logs out. It contains informations about the User, such as e-mail and role.
...
\textbf{Validity Token} This token is used by the platfrom to understand of the request to evaluate some code is legitimate or not. Once registering the repo to the platform, this token is generated and saved in the repo. It contains the Tournament end date, the Battle.
All of the above tokens have to be encripted to enshure their security and autenticity.
