...
This interaction is intended as placed everywhere there is a communication between components within the application server, so Controller to Microservice and between Microservices.\\
\useSvgWithCaption{./Images/sequenceDiagrams/dispatcherSequenceDiagram.svg}{.60}{.60}{Dispatcher Sequence Diagram}
Where ComponentA and ComponentB are two generic components, and Dispatcher is the dispatcher component.
...
This interaction is intended as placed everywhere there is a communication between an external service, or client and the Web server module.
...
Where the API call is any call as described above and defined in \nameref{reff:apicalls}, and request is any action the Platform has to perform as result of that call.
...
\end{itemize}
All of the following have a common argument, that is the User session token. This token is used to authenticate the User and to check if he has the right to perform the call. So all the following calls are valid if and only if the User is logged in.
\begin{itemize}
...
\textbf{Validity Token} This token is used by the platform to understand of the request to evaluate some code is legitimate or not. Once registering the repo to the platform, this token is generated and saved in the repo. It contains the Tournament end date, the Battle. \\
